        ...
        <div className="flex items-center justify-between h-16 md:h-20 gap-4">
          <a href="#home" className="flex items-center gap-2 shrink-0">
            ...
            <div className="leading-tight">
              ...
              <div className={`text-[11px] uppercase tracking-wider hidden sm:block ${scrolled ? 'text-slate-500' : 'text-white/80'}`}>Fresh · Reliable · 24/7</div>
            </div>
          </a>
          ...
          <div className="hidden md:flex flex-1 max-w-sm mx-auto xl:ml-auto xl:mr-0">
            <div className="relative w-full">
              <Search className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none ${scrolled ? 'text-slate-400' : 'text-white/70'}`} />
              <Input
                data-testid="product-search"
                value={q}
                onChange={(e) => { setQ(e.target.value); if (e.target.value) scrollToProducts() }}
                placeholder="Search products…"
                className={`h-10 pl-10 pr-9 rounded-full text-sm transition-all ${scrolled ? 'bg-slate-100 border-slate-200 text-slate-900 placeholder:text-slate-500 focus-visible:ring-brand-green' : 'bg-white/15 border-white/25 text-white placeholder:text-white/70 backdrop-blur focus-visible:ring-white/40'}`}
              />
              {q && (
                <button onClick={() => setQ('')} className={`absolute right-2.5 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full flex items-center justify-center ${scrolled ? 'text-slate-500 hover:bg-slate-200' : 'text-white/80 hover:bg-white/20'}`} aria-label="Clear">
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <button onClick={() => setMobileSearch(!mobileSearch)} className={`md:hidden p-2 rounded-md ${scrolled ? 'text-slate-800' : 'text-white'}`} aria-label="Search">
              <Search className="w-5 h-5" />
            </button>
            <button onClick={() => setOpen(!open)} className={`p-2 rounded-md ${scrolled ? 'text-slate-800' : 'text-white'}`} aria-label="Menu">
          ...
          <div className="md:hidden pb-3">
            <div className="relative">
              ...
              <Input
                ...
                onChange={(e) => { setQ(e.target.value); if (e.target.value) scrollToProducts() }}
                placeholder="Search products…"
                className="h-11 pl-10 pr-9 rounded-full text-sm bg-white border-slate-200 shadow"
              />
              {q && (
                <button onClick={() => setQ('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full flex items-center justify-center text-slate-500 hover:bg-slate-100" aria-label="Clear">
                  <X className="w-3.5 h-3.5" />
                </button>
              )}
            </div>
...
function Products({ q, setQ }) {
  const query = q.trim().toLowerCase()

  // Combine both categories for search matching, but only render poultry section here.
  // Beverages are shown in the Featured Brands section above.
  ...
  const beverageMatches = filteredBeverage.length
  const totalResults = filteredPoultry.length + beverageMatches
    ...
    <section id="products" className="py-20 lg:py-28 bg-slate-50 scroll-mt-24">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto">
          ...
          <h2 className="text-3xl lg:text-4xl font-bold text-slate-900">Fresh Poultry — Farm Direct</h2>
          <p className="mt-3 text-slate-600">Retail and wholesale poultry supply. Use the search bar in the top navigation to find any product instantly.</p>
          {query && (
            <div className="mt-4 text-sm text-slate-500">
              Showing results for <span className="font-semibold text-slate-800">&ldquo;{q}&rdquo;</span> — {totalResults} match{totalResults !== 1 ? 'es' : ''}
              {beverageMatches > 0 && (
                <> · <a href="#brands" className="text-brand-green underline">{beverageMatches} beverage brand{beverageMatches > 1 ? 's' : ''}</a> above</>
              )}
              {' '}
              <button onClick={() => setQ('')} className="text-brand-red hover:underline font-medium">clear</button>
            </div>
          )}
        </div>
        ...
        {filteredPoultry.length > 0 && (
          <div className="mt-12">
            ...
            <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.05 }}
              ...
              {filteredPoultry.map((p) => <ProductCard key={p.name} p={p} />)}
            </motion.div>
          </div>
        )}
      </div>
